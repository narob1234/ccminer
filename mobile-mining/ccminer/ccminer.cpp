...
int share_result(int result, int pooln, double sharediff, const char *reason)
{
	...
	applog(LOG_NOTICE, "\033[32mAccepted\033[0m:\033[32m[%lu]\033[0m/\033[0m\033[31m[%lu]\033[0m \033[93m%s\033[0m ⚡\033[1;96m%s %s%s\033[0m 🚀",
			p->accepted_count,
			p->rejected_count,
			suppl, s, flag, solved);
	if (reason) {
		...
		if (opt_algo == ALGO_CRYPTONIGHT || opt_algo == ALGO_CRYPTOLIGHT || opt_algo == ALGO_EQUIHASH) {
			...
			if (memcmp(&work.data[wcmpoft], &g_work.data[wcmpoft], wcmplen)) {
				...
				if (nicehash) {
					...
					// keep  xnsub as-is when not using nicehash mode
					opt_extranonce = opt_extranonce;
				}
			}
...
int main(int argc, char *argv[])
{
	...
	if (opt_algo == ALGO_EQUIHASH && !nicehash) {
		opt_extranonce = true; // enable subscribe by default.
	}
